...
export function PurchaseForm({ poolId, available, unitPrice, status, paymentsEnabled }: { poolId: string; available: number; unitPrice: string; status: string; paymentsEnabled: boolean }) {
  const [message, setMessage] = useState(''); const [pix, setPix] = useState(''); const [loading, setLoading] = useState(false);
  const enabled = status === 'aberto' && available > 0 && paymentsEnabled;
