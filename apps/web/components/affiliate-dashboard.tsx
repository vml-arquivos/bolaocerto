...
type Pool = { id: string; grupoId: string; grupo?: { id: string; nome: string; slug: string } | null; status: string; totalCotas: number | null; cotasIlimitadas?: boolean; cotasVendidas: number; cotasDisponiveis: number | null; valorCota: string; taxaAdministracaoPct: string; jogos?: Array<{ numeros: number[]; custo: string }>; concurso?: { modalidade: string; numeroConcurso: number; dataSorteio: string; cutoffAt: string; valorEstimadoPremio?: string | null } | null };
...
export function AffiliateDashboard() {
  ...
  return <div className="affiliate-page">
    ...
    {tab === 'boloes' && <div className="affiliate-workspace-grid"><section className="affiliate-table-card"><PoolWizard contests={openContests} groups={workspace.grupos} groupId={workspace.grupos[0]?.id ?? null} role="affiliate" editPool={editingPool} onCancelEdit={() => setEditingPool(null)} onSuccess={async () => { setEditingPool(null); await refresh(); }} /></section><section className="affiliate-table-card"><div className="affiliate-card-heading"><div><h2>Meus bolões</h2><p>Ações disponíveis somente sobre recursos do seu workspace.</p></div></div>{ownPools.length ? <div className="affiliate-list-cards">{ownPools.map((pool) => <article key={pool.id}><div className="pool-card-topline"><strong>{modalityLabels[pool.concurso?.modalidade ?? ''] ?? pool.concurso?.modalidade ?? 'Loteria'} · #{pool.concurso?.numeroConcurso ?? '—'}</strong><span className={`affiliate-status ${pool.status}`}>{statusText(pool.status)}</span></div><strong>{pool.cotasIlimitadas ? 'Cotas ilimitadas' : `${pool.totalCotas ?? 0} cotas`} · {money(pool.valorCota)}</strong><small>{pool.cotasIlimitadas ? 'Disponibilidade sob demanda' : `${pool.cotasDisponiveis ?? 0} disponíveis`} · {pool.jogos?.length ?? 0} jogo(s) · Grupo {pool.grupo?.nome ?? '—'}</small><div className="affiliate-row-actions">{['rascunho', 'aberto'].includes(pool.status) && <button onClick={() => setEditingPool(pool)}>Editar</button>}{pool.status === 'rascunho' && <button onClick={() => void mutate(`/api/v1/afiliados/me/boloes/${pool.id}/publicar`, { method: 'POST' }, 'Bolão publicado na vitrine.')}>Publicar</button>}{pool.status === 'aberto' && <button onClick={() => void mutate(`/api/v1/afiliados/me/boloes/${pool.id}/fechar`, { method: 'POST' }, 'Bolão fechado.')}>Fechar</button>}<button onClick={() => void mutate(`/api/v1/afiliados/me/boloes/${pool.id}/duplicar`, { method: 'POST' }, 'Bolão duplicado como rascunho.')}>Duplicar</button>{!['registrado', 'apurado', 'cancelado'].includes(pool.status) && <button onClick={() => { if (window.confirm('Cancelar este bolão?')) void mutate(`/api/v1/afiliados/me/boloes/${pool.id}`, { method: 'DELETE' }, 'Bolão cancelado.') }}>Cancelar</button>}</div></article>)}</div> : <div className="affiliate-empty"><strong>Nenhum bolão próprio.</strong><p>Crie um grupo e escolha um concurso para montar o primeiro.</p></div>}</section></div>}
