...
function AdminDashboardContent() {
  ...
  function PoolsView() {
    return <><SectionHeading eyebrow="Catálogo operacional" title="Bolões" description="Crie, revise, publique e acompanhe o ciclo de cada bolão." /><div className="admin-mini-stats"><span>Total <strong>{items.length}</strong></span><span>Rascunhos <strong>{items.filter((item) => item.status === 'rascunho').length}</strong></span><span>Abertos <strong>{items.filter((item) => item.status === 'aberto').length}</strong></span><span>Fechados <strong>{items.filter((item) => item.status === 'fechado').length}</strong></span></div><Panel title="Gerador profissional de bolão" description="Wizard em etapas com múltiplos jogos e resumo financeiro calculado no backend."><PoolWizard contests={contests} groupId={groupId} onSuccess={load} /></Panel><Panel title="Bolões cadastrados" description="Dados operacionais e disponibilidade de cotas"><Table minWidth={1260}><thead><tr><th>Modalidade</th><th>Concurso</th><th>Jogos</th><th>Cotas</th><th>Reservadas</th><th>Pagas</th><th>Disponíveis</th><th>Valor cota</th><th>Receita prevista</th><th>Cutoff</th><th>Status</th><th>Ações</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{modalityNames[item.concurso?.modalidade] ?? item.concurso?.modalidade ?? '—'}</td><td>#{item.concurso?.numeroConcurso ?? '—'}</td><td>{item.indicadores?.jogos ?? item.jogos?.length ?? 0}</td><td>{item.totalCotas}</td><td>{item.indicadores?.reservadas ?? 0}</td><td>{item.indicadores?.pagas ?? 0}</td><td><strong>{item.indicadores?.disponiveis ?? 0}</strong></td><td>{money(item.valorCota)}</td><td>{money(item.indicadores?.receitaPrevista)}</td><td>{dateTime(item.concurso?.cutoffAt)}</td><td><span className={statusClass(item.status)}>{statusLabel(item.status)}</span></td><td><div className="admin-row-actions"><Link className="admin-text-link" href={`/boloes/${item.id}`}>Ver</Link><button className="admin-button admin-button-small admin-button-ghost" onClick={() => execute(`/api/v1/admin/boloes/${item.id}/duplicar`, { method: 'POST' }, 'Bolão duplicado como rascunho.')}>Duplicar</button>{item.status === 'rascunho' && <button className="admin-button admin-button-small" onClick={() => execute(`/api/v1/admin/boloes/${item.id}/publicar`, { method: 'POST' }, 'Bolão publicado.')}>Publicar</button>}{item.status === 'aberto' && <button className="admin-button admin-button-small" onClick={() => execute(`/api/v1/admin/boloes/${item.id}/fechar`, { method: 'POST' }, 'Bolão fechado.')}>Fechar</button>}{!['registrado', 'apurado', 'cancelado'].includes(item.status) && <button className="admin-text-button" onClick={() => { if (window.confirm('Cancelar este bolão?')) void execute(`/api/v1/admin/boloes/${item.id}`, { method: 'DELETE' }, 'Bolão cancelado.'); }}>Cancelar</button>}</div></td></tr>)}</tbody></Table>{!items.length && <Empty title="Nenhum bolão cadastrado" description="Use o wizard acima para criar o primeiro bolão com vários jogos." />}</Panel></>;
  }
